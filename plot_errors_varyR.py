...
def plot_errors(filenames):
    ...
    axes[0][0].set_title("Large R (R=1.0 | Q=0.5)")
    ...
    axes[0][1].set_title("Small R (R=0.1 | Q=0.5)")
    ...
    axes[0][2].set_title("Best Q and R (R=1.0 | Q=1.0)")
    ...
    fig.suptitle("Spiral Trajectory (Top) and Time Data (Bottom) with Varying R and Best Q/R Combo", fontsize=16, fontweight='bold')
    ...
    fig.legend(handles=[plt0_odom, plt0_ekf, plt0_odomx, plt0_odomy, plt0_kfx, plt0_kfy], 
               labels=['Odometry (Measured)', 'EKF (Estimated)', 'EKF x (m)', 'EKF y (m)', 'Odom x (m)', 'Odom y (m)'], loc='lower right', fontsize=7)
